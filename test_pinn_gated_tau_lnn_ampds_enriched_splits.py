"""
Physics-Informed Gated-Tau LNN (PINN-GatedTau-LNN) for NILM
-- AMPds enriched splits (P + Q).

Based on test_pinn_basic_lnn_ampds_enriched_splits.py.

Key change: the static time-constant τ is replaced by a context-aware gate
that adapts the ODE integration speed to the current signal dynamics.

Strategy 2 — Gated Time-Constants (SSM-LNN Hybrid)
====================================================================
Standard LNNs use a fixed (or slowly learned) scalar per hidden unit:

    τ_i  (learnable parameter, same for every time step)

NILM has two very different regimes:
  • Transient  — device switches ON/OFF → sharp power spike, fast dynamics
  • Steady state — device running or idle → slow drift, long memory needed

A single static τ is a trade-off that serves neither regime well.

Here we replace the static τ with a dynamic gate that is a function of
both the current input and the previous hidden state:

    Gate output:     g_t = W_τ · cat(h_{t-1}, x_t) + b_τ     (hidden-dim)
    Dynamic τ:       τ_t = softplus(τ_base) · 2·sigmoid(g_t)

  • softplus(τ_base) provides a positive, learnable baseline (init ≈ 1.31)
  • 2·sigmoid(g_t):  gate in (0, 2); at init (zero weights) = 2·0.5 = 1.0
  → at init τ_t = softplus(τ_base) * 1.0  ≡ base LNN  (safe warm start)
  → after training: small τ for fast reaction (transients), large τ for
    slow integration (steady state)

The LNN ODE is otherwise unchanged:

    f_t = tanh( LN( W_in·x_t  +  W_rec·h ) )
    dh  = (−h / τ_t  +  f_t) · dt

Contrast with base LNN:
  Base   — τ is a fixed parameter vector, same across all time steps
  Gated  — τ_t adapts per step to instantaneous input + state dynamics

Initialisation: W_τ is zero-initialised so the model starts identically
to the base BasicLNN and the gate gradually activates through BPTT.

Architecture change summary:
    OLD (base):
        self.tau          = Parameter(ones(hidden))         [+64 params]
        tau = softplus(tau)
        dh  = (-h / tau + f_t) * dt

    NEW (gated):
        self.tau_base     = Parameter(ones(hidden))         [+64 params]
        self.tau_gate     = Linear(hidden+input, hidden)    [+4,288 params]
            zero-init weight + bias
        g_t  = tau_gate(cat(h, x_t))
        τ_t  = softplus(tau_base) * 2 * sigmoid(g_t)
        dh   = (-h / τ_t + f_t) * dt

    Net overhead vs base: +4,288 parameters  (64→4,352 for time-constant)
    Total: ~9,156 parameters  (vs 4,868 base)
...
class PhysicsInformedGatedTauLiquidNetworkModel(nn.Module):
    """
    BasicLNN with a context-aware dynamic time-constant (Gated-Tau).

    At each time step t the cell computes:

        g_t   = tau_gate( cat(h_{t-1}, x_t) )     -- raw gate logits
        τ_t   = softplus(tau_base) * 2*sigmoid(g_t) -- dynamic time-constant
        f_t   = tanh(LN(W_in·x_t + W_rec·h))
        dh    = (−h / τ_t + f_t) · dt
        h_new = clamp(h + dh, −10, 10)

    tau_gate is zero-initialised so at epoch 0:
        g_t = 0  →  sigmoid(0) = 0.5  →  2*0.5 = 1.0
        τ_t = softplus(tau_base) * 1.0  ≡  base BasicLNN
    """
    ...
    def __init__(self, input_size, hidden_size, n_appliances, dt=0.1):
        ...
        # ── Gated time-constant ──
        # Baseline: learnable positive scalar per hidden unit (same as base LNN)
        self.tau_base = nn.Parameter(torch.ones(hidden_size))
        # Gate: takes [h_{t-1}, x_t] → modulation factor for tau
        # Zero-init weight + bias ensures gate = 1.0 at initialisation
        self.tau_gate = nn.Linear(hidden_size + input_size, hidden_size)
    ...
    def forward(self, x):
        ...
        tau_base  = F.softplus(self.tau_base).unsqueeze(0)   # (1, hidden)

        for t in range(seq_len):
            x_t = x[:, t, :]                                # (batch, input_size)

            # Dynamic time-constant: adapts to current input + hidden state
            gate_input = torch.cat([h, x_t], dim=-1)        # (batch, hidden+input)
            g_t        = self.tau_gate(gate_input)           # (batch, hidden)
            tau_t      = tau_base * (2.0 * torch.sigmoid(g_t))  # (batch, hidden)
...
def train_pinn_model(data_dict, save_dir,
    ...
    # ── Model + losses ──
    model = PhysicsInformedGatedTauLiquidNetworkModel(
    ...
    print(f"Model parameters: {n_params:,}  "
          f"[LNN cell + tau_gate({hidden_size}+{INPUT_SIZE}→{hidden_size}) "
          f"+ tau_base + {len(APPLIANCES)} heads]")
    print("Starting PINN-GatedTau-LNN training (all appliances simultaneously)...")
    ...
    config = {
        ...
        'model':       'PhysicsInformedGatedTauLiquidNetworkModel',
        'description': (
            'BasicLNN with gated dynamic time-constant: '
            'τ_t = softplus(tau_base) * 2*sigmoid(tau_gate(cat(h,x_t))), '
            'zero-init gate, input=[P,Q]'),
        'loss':        (
        ...
        'architecture': {
            'cell':      'BasicLNN with gated tau (no static tau parameter)',
            'tau_base':  f'Parameter(ones({hidden_size}))  — positive baseline tau',
            'tau_gate':  (f'Linear({hidden_size}+{INPUT_SIZE}→{hidden_size}), zero-init  '
                          f'— context-aware tau modulation'),
    ...
    with open(os.path.join(save_dir, 'pinn_gated_tau_lnn_ampds_enriched_results.json'),
...
def _plot_training(history, test_metrics, save_dir):
    ...
    plt.title('Total Loss  [GatedTau-LNN]')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_gated_tau_lnn_ampds_enriched_loss.png'),
                dpi=150, bbox_inches='tight')
    plt.close()

    fig, axes = plt.subplots(len(APPLIANCES), 2,
                             figsize=(12, 4 * len(APPLIANCES)))
    fig.suptitle('PINN-GatedTau-LNN AMPds enriched (P+Q) -- Per-Appliance Val Metrics',
                 fontsize=13)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_gated_tau_lnn_ampds_enriched_per_appliance.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
...
if __name__ == "__main__":
    ...
    save_dir  = f"models/pinn_gated_tau_lnn_ampds_enriched_{timestamp}"
