...
def load_ampds_specific_splits():
    print("Loading AMPds data with specific splits...")

    with open('data/AMPds/train.pkl', 'rb') as f:
        train_data = pickle.load(f)[0]
    with open('data/AMPds/val.pkl', 'rb') as f:
        val_data = pickle.load(f)[0]
    with open('data/AMPds/test.pkl', 'rb') as f:
...
def train_on_appliance(data_dict, appliance_name, window_size=100,
                       ...
                       save_dir='models/conductance_lnn_ampds_specific'):
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(
                save_dir, f"conductance_lnn_ampds_{appliance_name.replace(' ', '_')}_best.pth")
            save_model(model,
    ...
    plt.savefig(os.path.join(save_dir,
        f"conductance_lnn_ampds_{appliance_name.replace(' ', '_')}_metrics.png"),
    ...
    with open(os.path.join(save_dir,
            f'conductance_lnn_ampds_{appliance_name.replace(" ", "_")}_history.json'),
...
def test_on_all_appliances(window_size=100, hidden_size=64, dt=0.1,
                           epochs=80, lr=0.001, patience=20):
    data_dict = load_ampds_specific_splits()
            ...
            if model is not None:
                all_results[appliance_name] = {
                    'model_path': os.path.join(
                        appliance_dir,
                        f"conductance_lnn_ampds_{appliance_name.replace(' ', '_')}_best.pth"),
    ...
    print(f"\nConductanceLTCModel AMPds testing completed. Results saved to {base_save_dir}\n")
...
if __name__ == "__main__":
    print("Testing ConductanceLTCModel on AMPds dataset with specific splits...")

    for f in ['data/AMPds/train.pkl', 'data/AMPds/val.pkl', 'data/AMPds/test.pkl']:
    ...
    print(f"\nSummary of ConductanceLTCModel testing on AMPds dataset:")
