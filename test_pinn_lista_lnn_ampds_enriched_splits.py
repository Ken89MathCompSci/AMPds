"""
PINN-LISTA-LNN for NILM -- AMPds enriched splits (P + Q).

Same structure as test_pinn_lista_lnn_redd_specific_splits.py but uses the
enriched AMPds dataset (data/AMPds_enriched_data/) with two input channels:
  channel 0 : main   -- mains active power  (W)
  channel 1 : main_Q -- mains reactive power (VAR)

Appliances: dish washer, fridge, oven, washer dryer  (AMPds2)

Architecture:
    y  (batch, WIN, 2)  -- scaled [P, Q] window
     |
     +-- LNN encoder: BasicLiquidTimeLayer sees both P and Q
     |       (lnn_input: Linear(2, hidden))
     |   --> h  (batch, hidden)
     |
     +-- LISTA sparse decomposition: channel 0 (P) only
     |       y_flat = y[:, :, 0]   (batch, WIN)
     |   x^(0) = h_to_x0(h)       (warm start from LNN hidden state)
     |   x^(K) via K LISTALayers   (batch, n_atoms)
     |
     +-- 4 x Linear heads  -->  p_hat  (batch, 4)
     |
     Dictionary D (n_atoms, WIN) jointly learned for P-channel reconstruction.

Rationale for P-only LISTA:
    LISTA reconstructs the active power signal and decomposes it into a sparse
    appliance basis. Q carries load-type information (used by the LNN encoder)
    but is not directly decomposable by the same energy-balance dictionary.

Loss:
    Stage 1 (epochs 1-WARMUP_EPOCHS):
        MSE + lambda_recon * Recon(P) + lambda_sparse * L1
    Stage 2 (remaining epochs):
        + lambda_phys * L_phys + per-appliance BCE
"""
...
BCE_LAMBDA = {'dish washer': 0.3, 'fridge': 0.3, 'basement': 0.3,  'heat pump': 0.5}
BCE_ALPHA  = {'dish washer': 1.5, 'fridge': 1.5, 'basement': 4.0,  'heat pump': 5.0}
...
class PhysicsConsistencyLoss(nn.Module):
    ...
    x_mid_scaled is channel 0 (active power P) at the window midpoint.
    ...
    def __init__(self, x_scaler_P, y_scalers, appliances, epsilon_w=EPSILON_W):
        ...
        self.register_buffer('x_min',   torch.tensor(float(x_scaler_P.data_min_[0]),   dtype=torch.float32))
        self.register_buffer('x_range', torch.tensor(float(x_scaler_P.data_range_[0]), dtype=torch.float32))
...
class PINNLISTALNNModel(nn.Module):
    """
    LNN temporal encoder (input_size=2) + LISTA sparse refinement (P channel)
    + per-appliance heads.

    Forward pass:
        1. BasicLiquidTimeLayer processes (batch, WIN, 2) sequentially
           --> hidden state h  (batch, hidden_size)
        2. h projected to x^(0) as LISTA warm start
        3. K LISTA layers refine x^(0) using y_flat = y[:, :, 0]  (P channel)
           --> x^(K)  (batch, n_atoms)
        4. Per-appliance linear heads --> p_hat  (batch, n_appliances)

    LNN cell (Basic, fixed tau):
        tau   = softplus(tau_param)
        f_t   = tanh(W_in * x_t + W_rec * h)    x_t has 2 features
        dh    = (-h / tau + f_t) * dt
    ...
    def __init__(self, input_size: int = INPUT_SIZE,
        ...
        # LNN encoder — accepts full input_size (P + Q)
        self.lnn_input = nn.Linear(input_size, hidden_size)
        ...
        # Warm-start projection from LNN hidden state to LISTA code
        self.h_to_x0 = nn.Linear(hidden_size, n_atoms)

        # LISTA layers — work on the P channel signal of length signal_len
        self.lista_layers = nn.ModuleList([
            LISTALayer(signal_len, n_atoms) for _ in range(k_layers)
        ])

        # Dictionary for P-channel reconstruction  (n_atoms, signal_len)
    ...
    def forward(self, y: torch.Tensor):
        ...
        # LNN encoder sees both channels
        ...
        # LISTA refines over P channel only
        x      = self.h_to_x0(h)
        y_flat = y[:, :, 0]                              # (batch, WIN) — P channel
    ...
    def reconstruct(self, x: torch.Tensor) -> torch.Tensor:
        """Reconstruct P-channel signal from sparse code."""
        return x @ self.D
...
def train_pinn_lista_model(data_dict, save_dir,
    ...
    print(f"input_size={INPUT_SIZE}  hidden={hidden_size}  dt={dt}  "
          f"n_atoms={n_atoms}  K={k_layers}  "
          f"lambda_recon={lambda_recon}  lambda_sparse={lambda_sparse}  "
          f"lambda_phys={lambda_phys}  epsilon={epsilon_w}W")
    ...
    # Per-channel scaling; x_scalers[0] = P scaler for physics loss
    ...
    # Physics loss uses P-channel scaler (x_scalers[0])
    ...
    for epoch in range(EPOCHS):
        ...
        for xb, yb in bar:
            ...
            # Reconstruct P channel only
            y_flat     = xb[:, :, 0]                     # (batch, WIN)
            ...
            if epoch < WARMUP_EPOCHS:
                ...
                x_mid     = xb[:, WIN // 2, 0]           # P channel midpoint
    ...
    config = {
        ...
        'model':       'PINNLISTALNNModel',
        'description': ('BasicLNN encoder (input=[P,Q]) warm-starts K-layer LISTA '
                        '(P channel) + PhysicsConsistency (sum_p_hat <= P_agg + epsilon)'),
        'loss':        'MSE + lambda_recon*Recon(P) + lambda_sparse*L1 + lambda_phys*Phys [+ BCE stage2]',
        'input_size':  INPUT_SIZE,
        'window_size': WIN,
        'model_params': {
            'input_size': INPUT_SIZE, 'signal_len': WIN,
            'hidden_size': hidden_size, 'dt': dt,
            'n_atoms': n_atoms, 'k_layers': k_layers,
            'n_appliances': len(APPLIANCES),
        },
    ...
    with open(os.path.join(save_dir, 'pinn_lista_lnn_ampds_enriched_results.json'),
...
def _plot_training(history, test_metrics, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lista_lnn_ampds_enriched_loss.png'),
                dpi=150, bbox_inches='tight')
    plt.close()

    fig, axes = plt.subplots(len(APPLIANCES), 2,
                             figsize=(12, 4 * len(APPLIANCES)))
    fig.suptitle('PINN-LISTA-LNN AMPds enriched (P+Q) -- Per-Appliance Val Metrics',
                 fontsize=13)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lista_lnn_ampds_enriched_per_appliance.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
...
if __name__ == "__main__":
    ...
    save_dir  = f"models/pinn_lista_lnn_ampds_enriched_{timestamp}"
