"""
Physics-Informed Basic LNN (PINN-BasicLNN) for NILM -- AMPds enriched splits (P + Q).

Same structure as test_pinn_basic_lnn_ukdale_specific_splits.py but uses the
...
Appliances: dish washer, fridge, oven, washer dryer  (AMPds2 meter map)
...
Architecture:
    ...
    LayerNorm(hidden)
         |
    +----+----+----+----+
    | DW | FR | OV | WD |  -- one Linear head per appliance
    +----+----+----+----+
    output: (batch, 4)
...
def train_pinn_model(data_dict, save_dir,
    ...
    config = {
        ...
        'model':       'PhysicsInformedBasicLiquidNetworkModel',
        'description': 'basic LNN (fixed tau, no gate) + per-appliance heads + L_phys, input=[P,Q]',
    ...
    with open(os.path.join(save_dir, 'pinn_basic_lnn_ampds_enriched_results.json'),
...
def _plot_training(history, test_metrics, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_basic_lnn_ampds_enriched_loss.png'),
                dpi=150, bbox_inches='tight')
    plt.close()

    fig, axes = plt.subplots(len(APPLIANCES), 2,
                             figsize=(12, 4 * len(APPLIANCES)))
    fig.suptitle('PINN-BasicLNN AMPds enriched (P+Q) -- Per-Appliance Val Metrics',
                 fontsize=13)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_basic_lnn_ampds_enriched_per_appliance.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
...
if __name__ == "__main__":
    ...
    save_dir  = f"models/pinn_basic_lnn_ampds_enriched_{timestamp}"
