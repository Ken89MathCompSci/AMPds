"""
Physics-Informed Huber LNN (PINN-HuberLNN) for NILM -- AMPds enriched splits (P + Q).

Identical to test_pinn_basic_lnn_ampds_enriched_splits.py except the physics
consistency loss uses a Huber-style penalty instead of plain ReLU:

    violation = ReLU(sum_i p_hat_i_raw - P_agg_raw - epsilon)
    L_phys    = huber_loss(violation, 0,  delta=HUBER_DELTA)

Behaviour:
    |violation| <= delta : squared penalty  (strong push, sensitive to small errors)
    |violation|  > delta : linear penalty   (gradient capped, resistant to outlier batches)

This prevents a single extreme batch (e.g. heat pump surge) from dominating
the physics gradient while still correcting moderate violations quadratically.
"""
...
LAMBDA_PHYS   = 0.01
EPSILON_W     = 50.0
HUBER_DELTA   = 100.0    # watts -- squared below, linear above
WARMUP_EPOCHS = 20
...
class PhysicsConsistencyLoss(nn.Module):
    """
    One-sided Huber penalty on energy conservation:

        violation = ReLU(sum_i p_hat_i_raw - P_agg_raw - epsilon)
        loss      = huber_loss(violation, 0, delta=huber_delta)

    Quadratic for violations <= huber_delta watts, linear beyond.
    This caps the gradient contribution of extreme outlier batches.
    """
...
def train_pinn_model(data_dict, save_dir,
    ...
    print("Starting PINN-HuberLNN training (all appliances simultaneously)...")
    ...
    with open(os.path.join(save_dir, 'pinn_huber_lnn_ampds_enriched_results.json'),
...
def _plot_training(history, test_metrics, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_huber_lnn_ampds_enriched_loss.png'),
                dpi=150, bbox_inches='tight')
    plt.close()

    fig, axes = plt.subplots(len(APPLIANCES), 2,
                             figsize=(12, 4 * len(APPLIANCES)))
    fig.suptitle('PINN-HuberLNN AMPds enriched (P+Q) -- Per-Appliance Val Metrics',
                 fontsize=13)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_huber_lnn_ampds_enriched_per_appliance.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
...
if __name__ == "__main__":
    ...
    save_dir  = f"models/pinn_huber_lnn_ampds_enriched_{timestamp}"
