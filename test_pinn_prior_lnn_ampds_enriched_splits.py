"""
Physics-Informed Prior LNN (PINN-Prior-LNN) for NILM
-- AMPds enriched splits (P + Q).

Based on test_pinn_basic_lnn_ampds_enriched_splits.py.

Key change: the physics constraint is embedded INSIDE the LNN's ODE dynamics
as a continuous conservation residual, rather than only as an external penalty.

Idea 3 — Incorporating Physical Priors in the Differential Equation
====================================================================
In the base PINN the physics law (sum of appliance powers ≤ aggregate) is
enforced only at the OUTPUT after the full sequence is unrolled:

    L_phys = ReLU(Σ p̂_i_raw − P_agg_raw − ε)   ← external penalty

Here we additionally modify the LNN cell so the hidden state h(t) is
continuously driven toward states that are consistent with the observed
aggregate power at EVERY time step:

    1. Physics probe  : p̂_agg_t = W_probe · h_{t-1}   (scalar, scaled)
       Learns to estimate the aggregate active power from the hidden state.

    2. Conservation residual  : r_t = x_t[:,0] − p̂_agg_t
       Positive when the hidden state under-estimates aggregate (needs to
       increase power allocation); negative when it over-estimates.

    3. Physics forcing : φ_t = W_phys · r_t   (hidden-space vector)
       Projects the residual back into hidden state space.

    4. Modified ODE cell:
           f_t = tanh( LN( W_in·x_t  +  W_rec·h  +  φ_t ) )
           dh  = (−h/τ + f_t) · dt

The forcing φ_t acts as a continuous corrective signal inside the cell:
if the current hidden state is not conserving energy, the residual drives
future hidden-state updates toward conservation before the sequence ends.

Contrast with external physics loss:
  External   — penalises FINAL hidden state that leads to sum > P_agg
  Prior ODE  — steers hidden-state TRAJECTORY at every step toward
               states consistent with instantaneous P_agg

Initialisation: W_phys is zero-initialised so the model starts as the
base BasicLNN and the physics prior is gradually activated through training.

Architecture change summary:
    OLD (base):
        LNN cell: f_t = tanh(LN(W_in·x + W_rec·h)) * dt

    NEW (prior):
        p̂_agg_t = W_probe · h                              [+65 params]
        r_t      = x_t[:,0:1] − p̂_agg_t
        φ_t      = W_phys · r_t                             [+65 params]
        f_t      = tanh(LN(W_in·x + W_rec·h + φ_t)) * dt
...
# ---------------------------------------------------------------------------
# Physics-Prior LNN Model
# ---------------------------------------------------------------------------

class PhysicsInformedPriorLiquidNetworkModel(nn.Module):
    """
    BasicLNN with a physics-prior term embedded in the ODE dynamics.

    At each time step t the cell computes:

        p̂_agg_t  = phys_probe(h_{t-1})         -- scalar P_agg estimate
        r_t       = x_t[:,0:1] − p̂_agg_t       -- conservation residual
        φ_t       = phys_proj(r_t)              -- forcing vector (hidden)
        f_t       = tanh(LN(W_in·x_t + W_rec·h + φ_t))
        dh        = (−h/τ + f_t) · dt
        h_new     = clamp(h + dh, −10, 10)

    phys_proj is zero-initialised so training begins identically to the base
    BasicLNN and the physics forcing is gradually activated through BPTT.
    """
    ...
    def __init__(self, input_size, hidden_size, n_appliances, dt=0.1):
        ...
        # ── Physics-prior components ──
        # Probe: lightweight estimate of scaled P_agg from hidden state
        self.phys_probe = nn.Linear(hidden_size, 1)
        # Projection: scalar conservation residual → hidden forcing vector
        # Zero-init: model starts as base BasicLNN; prior activates via BPTT
        self.phys_proj  = nn.Linear(1, hidden_size)
    ...
    def forward(self, x):
        ...
        for t in range(seq_len):
            ...
            # Physics prior: residual between observed P_agg and h's estimate
            p_est      = self.phys_probe(h)         # (batch, 1) — scaled
            phys_resid = x_t[:, 0:1] - p_est        # (batch, 1) — conservation error
            phys_force = self.phys_proj(phys_resid)  # (batch, hidden) — ODE forcing

            # LNN cell with physics forcing added to the pre-activation
            input_proj = self.input_proj(x_t)
            rec_proj   = torch.matmul(h, self.rec_weights)
            f_t        = torch.tanh(self.intra_norm(input_proj + rec_proj + phys_force))
...
def train_pinn_model(data_dict, save_dir,
    ...
    # ── Model + losses ──
    model = PhysicsInformedPriorLiquidNetworkModel(
        input_size=INPUT_SIZE, hidden_size=hidden_size,
        n_appliances=len(APPLIANCES), dt=dt,
    ).to(device)

    mse_criterion  = nn.MSELoss()
    # Physics loss uses channel 0 (active power P) scaler only
    ...
    print(f"Model parameters: {n_params:,}  "
          f"[LNN cell + phys_probe({hidden_size}→1) + phys_proj(1→{hidden_size}) "
          f"+ {len(APPLIANCES)} heads]")
    print("Starting PINN-Prior-LNN training (all appliances simultaneously)...")
    ...
    for epoch in range(EPOCHS):
        ...
        for xb, yb in progress_bar:
            ...
            # Physics constraint uses scaled P channel at window midpoint
    ...
    config = {
        ...
        'model':       'PhysicsInformedPriorLiquidNetworkModel',
        'description': (
            'BasicLNN with physics-prior ODE term: conservation residual '
            'r_t = P_agg_t − phys_probe(h_{t-1}) fed back into cell dynamics '
        ...
        'architecture': {
            ...
            'phys_proj':  f'Linear(1→{hidden_size})  — residual forcing, zero-init',
    ...
    with open(os.path.join(save_dir, 'pinn_prior_lnn_ampds_enriched_results.json'),
...
def _plot_training(history, test_metrics, save_dir):
    ...
    plt.title('Total Loss  [Prior-LNN]')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_prior_lnn_ampds_enriched_loss.png'),
                dpi=150, bbox_inches='tight')
    plt.close()

    fig, axes = plt.subplots(len(APPLIANCES), 2,
                             figsize=(12, 4 * len(APPLIANCES)))
    fig.suptitle('PINN-Prior-LNN AMPds enriched (P+Q) -- Per-Appliance Val Metrics',
                 fontsize=13)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_prior_lnn_ampds_enriched_per_appliance.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
...
if __name__ == "__main__":
    ...
    save_dir  = f"models/pinn_prior_lnn_ampds_enriched_{timestamp}"
